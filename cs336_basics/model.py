...
class Multihead_Self_Attention(nn.Module):
    def __init__(self,d_model:int,num_heads:int,rope=None):
        ...
        self.q_proj = Linear(d_model,d_model) #(d_model,h*d_k)
        self.k_proj = Linear(d_model,d_model) #(d_model,h*d_k)
        self.v_proj = Linear(d_model,d_model) #(h*d_v,d_model)
        self.output_proj = Linear(d_model,d_model) #(d_model,h*d_v)
        self.rope = rope
...
class Transformer_block(nn.Module):
    def __init__(self,d_model,num_heads,d_ff,theta,max_seq_len):
        ...
        self.rope = RotaryPositionEmbedding(theta=theta,d_k = d_model/num_heads,max_seq_len=max_seq_len)
        self.attn = Multihead_Self_Attention(d_model,num_heads,self.rope)
